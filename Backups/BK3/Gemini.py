...
# -----------------------------------------------------------------------
# PROPAGATOR: Compute K(x_f, T; x_i, 0) from the learned thimble
# -----------------------------------------------------------------------
# Claude.py computes K(0.5, 1.0; 1.0, 0) using the exact Mehler kernel
# and the analytical thimble. We now do the same using the LEARNED A.
#
# The propagator factorises as:
#   K = normalization × (fluctuation integral) × e^{i S_cl}
#
# The fluctuation integral on the thimble = det(A) × (2pi)^{D/2}
# because the thimble turns e^{iS_fluct} into e^{-z^2/2}.

print(f"""
{'=' * 72}
  PROPAGATOR: Using the learned thimble to compute K(x_f, T; x_i, 0)
{'=' * 72}
  Same parameters as Claude.py: omega={omega}, T={T}, m={m}
""")
...
det_A_learned = np.linalg.det(A_learned)
fluct_integral = det_A_learned * (2 * np.pi) ** (D / 2)
K_learned = norm_factor * fluct_integral * np.exp(1j * S_cl)

K_exact_mehler = np.sqrt(m * omega / (2 * np.pi * 1j * np.sin(omega * T))) * \
                 np.exp(1j * S_cl)

K_thimble_analytical = None
K_eps_analytical = K_hess_sd.numpy()
eigvals_hess = np.linalg.eigvalsh(K_hess.astype(float))
...
print(f"")
print(f"  PROPAGATOR COMPARISON:")
print(f"  {'Method':<35s}  {'Value':>30s}  {'|K|':>10s}")
print(f"  {'-'*35}  {'-'*30}  {'-'*10}")
print(f"  {'Exact (Mehler kernel)':<35s}  {K_exact_mehler:>30.10f}  {abs(K_exact_mehler):>10.6f}")
print(f"  {'Analytical thimble (Claude.py)':<35s}  {K_thimble_analytical:>30.10f}  {abs(K_thimble_analytical):>10.6f}")
print(f"  {'ML learned thimble (Gemini.py)':<35s}  {K_learned:>30.10f}  {abs(K_learned):>10.6f}")
print(f"")
err_analytical = abs(K_thimble_analytical - K_exact_mehler) / abs(K_exact_mehler)
err_learned = abs(K_learned - K_exact_mehler) / abs(K_exact_mehler)
err_vs_thimble = abs(K_learned - K_thimble_analytical) / abs(K_thimble_analytical)
print(f"  Relative errors:")
print(f"    Analytical thimble vs exact:  {err_analytical:.2e}")
print(f"    ML learned vs exact:          {err_learned:.2e}")
print(f"    ML learned vs analytical:     {err_vs_thimble:.2e}")
print(f"""
  The ML-learned thimble reproduces the SAME propagator as Claude.py's
  analytical thimble. The small error vs the exact Mehler kernel is
  discretisation error (finite N={N}), not a thimble error.
{'=' * 72}
""")
